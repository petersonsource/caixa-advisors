...
public class WorkItemSetCategoryValue_CopyBKP extends AbstractRTCService implements
	IOperationAdvisor {
	...
	try {
		...
		for (IProcessConfigurationElement configElement : configElements) {
		    ...
		    attrCategory = getAttr(workItemService, workItem,
		    ...
		    // if not change
		    if (oldWorkItem != null) {
			oldAttrCategory = getAttr(workItemService, oldWorkItem,
				condifAttrId, monitor);
