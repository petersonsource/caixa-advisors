...
public class WorkItemSetClassValue extends AbstractRTCService implements IOperationAdvisor {
			...
			try {
				...
				if (!projectList.getProjects().isEmpty()) {
					...
					if (apl.equalsIgnoreCase(projectList.getProjects().get(i).getName())) {
					    classificacao = projectList.getProjects().get(i).getClassificacao();
