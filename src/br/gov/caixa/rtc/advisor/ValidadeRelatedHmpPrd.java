...
public class ValidadeRelatedHmpPrd extends AbstractRTCService implements
	IOperationAdvisor {
	...
	if (data instanceof ISaveParameter) {
		...
		try {
			...
			String configHmpWiAttrValue = null;
			String configHmpWiAttrId = null;
			String configImplHmpWiAttrValue = null;
			String configImplHmpWiAttrId = null;
			...
			for (IProcessConfigurationElement configElement : configElements) {
			    ...
			    configWiRelatedType = configElement
				    .getAttribute("wiRelatedType");
			    configWiRelatedStateId = configElement
				    .getAttribute("wiRelatedStateId");

			    // Ambiente Homologação/Produção
			    configWiAttrId = configElement
				    .getAttribute("wiAttrId");
			    configWiAttrValue = configElement
				    .getAttribute("wiAttrValue");

			    // O sistema possui ambiente HMP?
			    configHmpWiAttrId = configElement
				    .getAttribute("hmpWiAttrId");
			    configHmpWiAttrValue = configElement
				    .getAttribute("hmpAttrValue");

			    // Informação sobre a implantação em HMP (TI)
			    configImplHmpWiAttrId = configElement
				    .getAttribute("implHmpWiAttrId");
			    configImplHmpWiAttrValue = configElement
				    .getAttribute("implHmpAttrValue");

			    // Clase: Não valida relacionado se Emergencial
			    configWiClassId = configElement
				    .getAttribute("wiClassId");
			    configWiClassValue = configElement
				    .getAttribute("wiClassValue");
			    ...
			    // valida valor da enumeracao O sistema possui
			    // ambiente HMP?
			    Object idValueHmp = getAttribute(workItem,
				    workItemService, configHmpWiAttrId, monitor);

			    if (!idValueHmp.equals(configHmpWiAttrValue)) {
				continue;
			    }

			    // valida valor da enumeracao implantação em HMP
			    // (TI)
			    Object idValueImplHmp = getAttribute(workItem,
				    workItemService, configImplHmpWiAttrId,
				    monitor);
			    if (!idValueImplHmp
				    .equals(configImplHmpWiAttrValue)) {
				continue;
			    }

			    // TODO LPF
			    // valida valor da enumeracao (Ex.: acelerado)
			    Object idClassValue = getAttribute(workItem,
				    workItemService, configWiClassId, monitor);
			    if (idClassValue.equals(configWiClassValue)) {
				continue;
			    }

			    // valida valor da enumeracao Homologação/Produção
			    Object idValue = getAttribute(workItem,
				    workItemService, configWiAttrId, monitor);
				...
				configWiRelatedStateIdAr = configWiRelatedStateId
					.toLowerCase().split(";");
				listaEstadosPermitidos = new HashSet<String>(
					Arrays.asList(configWiRelatedStateIdAr));
				if (listaEstadosPermitidos == null) {
				    String alert = Messages.getStringWithArgs(
					    Messages.MSG_ERRO_CONFIG_ESTADO,
					    "Valida Item Relacionado");
				    IAdvisorInfo info = collector
					    .createProblemInfo(alert, "",
						    ERROR_MESSAGE_TYPE);
				...
				configWiRelatedTypeAr = configWiRelatedType
					.toLowerCase().split(";");
				listaWIRelacionadoPermitidos = new HashSet<String>(
					Arrays.asList(configWiRelatedTypeAr));
				if (listaWIRelacionadoPermitidos == null) {
				    String alert = Messages
					    .getStringWithArgs(
						    Messages.MSG_ERRO_CONFIG_ITEM_RELACIONADO,
						    "Valida Item Relacionado");
				    IAdvisorInfo info = collector
					    .createProblemInfo(alert, "",
						    ERROR_MESSAGE_TYPE);
			    ...
			    references = workItemService
				    .resolveWorkItemReferences(workItem,
					    monitor);
			    ...
			    if (relatedReferences != null
				    && relatedReferences.size() > 0) {
				for (IReference relatedReference : relatedReferences) {
				    wiRelatedHandle = (IWorkItemHandle) relatedReference
					    .resolve();
				    workItemRelated = (IWorkItem) repository
					    .fetchItem(
						    wiRelatedHandle,
						    IWorkItem.MEDIUM_PROFILE
							    .getPropertiesArray());
					...
					if (!listaWIRelacionadoPermitidos
						.contains(workItemRelated
							.getWorkItemType()
							.toLowerCase())) {
					    relatedOutOfScope++;
					    continue;
					}
					if (!listaEstadosPermitidos
						.contains(workItemRelated
							.getState2()
							.getStringIdentifier()
							.toLowerCase())) {
					...
					if (relatedOutOfScope
						.equals(configWiRelatedTypeAr.length)) {
					    wiOutOfScope++;
					}
				...
				EnumerationDto valueEnum = (EnumerationDto) getFullAttribute(
					...
					configWiAttrId, monitor);
				String strValueEnum = valueEnum.getValue();
				String alert = Messages.getStringWithArgs(
					Messages.MSG_ERRO_RELACIONAMENTO_PRD,
					strValueEnum);

				IAdvisorInfo info = collector
					.createProblemInfo(
						alert,
						Messages.MSG_ALERTA_CONTATO_QUALIDADE,
						ERROR_MESSAGE_TYPE);
				collector.addInfo(info);
			    }
				...
				IAdvisorInfo info = collector
					.createProblemInfo(
						Messages.MSG_ERRO_ITENS_NAO_VALIDADOS,
						Messages.MSG_ALERTA_CONTATO_QUALIDADE,
						ERROR_MESSAGE_TYPE);
				collector.addInfo(info);
			    }
			    if (wiOutOfScope.equals(relatedReferences.size())
				    && relatedReferences.size() > 0) {
				IAdvisorInfo info = collector
					.createProblemInfo(
						Messages.MSG_ERRO_RELACIONAMENTO_ERRADO,
						Messages.MSG_ALERTA_CONTATO_QUALIDADE,
						ERROR_MESSAGE_TYPE);
				collector.addInfo(info);
    ...
    public static Object getFullAttribute(IWorkItem workItem,
